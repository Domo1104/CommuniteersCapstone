...
export default function SelectEvent(){
    ...
    useEffect(()=>{
        ...
        if(selectedEvent.length > 0){
            ...
            const signedUp = volunteerLogIn.events.filter((event)=> event.id === selectedEvent[0].id)
  ...
  return (
    <div className='join-event-card'>
        ...
        { !joinedEvent ? (
            <div>
                Role:  
            <EventTask category={category} selectedTask={selectedTask} setSelectedTask={setSelectedTask} />
        <br/>
        <br/>
        <button className='btn-join' onClick={joinEvent}>Join Event</button>
            </div>
